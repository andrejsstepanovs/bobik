...
class ToolLoader:
    ...
    def add_bing_search_tool(self, tools: List[Any]):
        if self.is_tool_enabled("bing_search") and self.config.bing_settings["subscription_key"] is not None:
            ...
            bing_api: BingSearchAPIWrapper = BingSearchAPIWrapper(
                bing_subscription_key=self.config.bing_settings["subscription_key"],
                bing_search_url=self.config.bing_settings["url"]["search"]
            )
            search: BingSearchResults = BingSearchResults(api_wrapper=bing_api)
            tools.append(search)

    def add_bing_news_tool(self, tools: List[Any]):
        if self.is_tool_enabled("bing_news") and self.config.bing_settings["subscription_key"] is not None:
            ...
            if self.config.bing_settings["url"]["news"]:
                news_search: news_tools.NewsRetrievalTool = news_tools.NewsRetrievalTool(
    ...
    def add_wttr_weather_tool(self, tools: List[Any]):
    ...
    def add_my_calendar_tool(self, tools: List[Any]):
        if self.is_tool_enabled("ics_calendar"):
            calendar_config_file: str = self.config.settings.tools.ics_calendar.config_file
            ...
            with open(calendar_config_file, "r") as file:
                ...
                calendar_options: Dict[str, Any] = yaml.safe_load(file)
                calendar: Calendar = Calendar(state=self.state, options=calendar_options)
                my_calendar_tool: calendar_tools.CalendarEventTool = calendar_tools.CalendarEventTool(calendar=calendar)
